...
const BAN_REASON_LABELS: Record<BanReason, string> = {
    ...
    REPEATED_CANCELS: "Repeated Cancels",
                            ...
                            <thead>
                                <tr className="bg-zinc-50/80">
                                    {["User", "Contact", "Status", "Joined", "Actions"].map((h) => (
                                        <th key={h} className="text-left px-4 py-3 text-[13px] font-medium text-zinc-700 tracking-wide">
                                            {h}
                                        </th>
                                    ))}
                                </tr>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button onClick={() => setDetailModal({ open: true, user: u })}
                                                    className="text-[11.5px] cursor-pointer font-semibold text-cyan-500 hover:text-cyan-600 border border-cyan-300 hover:border-cyan-300 bg-cyan-50 hover:bg-cyan-100 px-2.5 py-1.5 rounded-lg transition-all">
                                                    View
                                                </button>
                                                {/* ban / unban */}
                                                {u.isActive ? (
                                                    <button onClick={() => setBanModal({ open: true, user: u })}
                                                        className="text-[11.5px] cursor-pointer font-semibold text-red-500 hover:text-red-600 border border-red-200 hover:border-red-300 bg-red-50 hover:bg-red-100 px-2.5 py-1.5 rounded-lg transition-all">
                                                        Ban
                                                    </button>
                                                ) : (
                                                    <button onClick={() => handleUnban(u.id)}
                                                        className="text-[11.5px] cursor-pointer font-semibold text-emerald-600 hover:text-emerald-700 border border-emerald-200 hover:border-emerald-300 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 rounded-lg transition-all">
                                                        Unban
                                                    </button>
                                                )}
                                                {/* delete */}
                                                <button onClick={() => setDeleteModal({ open: true, user: u })} title="Delete user" className="text-red-500 hover:text-white border border-red-200 
                                                        hover:border-red-500 bg-red-50 hover:bg-red-500 p-1.5 rounded-lg cursor-pointer transition-all">
                ...
                {pagination && pagination.totalPages > 1 && (
                    <div className="flex items-center justify-between px-6 py-5 border-t border-zinc-100">
                        ...
                        <div className="flex items-center gap-1.5">
                            <button disabled={!pagination.hasPrev} onClick={() => setPage(p => p - 1)} className="w-7.5 h-7.5 rounded-lg cursor-pointer border border-zinc-200 flex items-center justify-center text-zinc-500 hover:border-cyan-300 hover:text-cyan-600 hover:bg-cyan-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            {Array.from({ length: pagination.totalPages }, (_, i) => i + 1)
                                ...
                                .map((p, idx, arr) => (
                                    <div key={p}>
                                        ...
                                        <button onClick={() => setPage(p)} className={`w-7.5 h-7.5 rounded-lg cursor-pointer text-[12.5px] font-semibold border transition-all 
                                            ${p === page ? "bg-cyan-500 text-white border-cyan-500" : "border-zinc-200 text-zinc-600 hover:border-cyan-300 hover:text-cyan-600 hover:bg-cyan-50"}`}>
                                            {p}
                                        </button>
                                    </div>
                                ))
                            ...
                            <button disabled={!pagination.hasNext} onClick={() => setPage(p => p + 1)}
                                className="w-7.5 h-7.5 rounded-lg cursor-pointer border border-zinc-200 flex items-center justify-center text-zinc-500 hover:border-cyan-300 hover:text-cyan-600 hover:bg-cyan-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 border border-zinc-100">
                        ...
                        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5 flex items-start gap-2.5">
                            <svg className="w-4.5 h-4.5 text-red-500 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {(Object.entries(BAN_REASON_LABELS) as [BanReason, string][]).map(([val, label]) => (
                                    <button key={val} onClick={() => setBanReason(val)}
                                        className={`py-2 px-3 rounded-xl text-[12px] font-medium border text-left transition-all 
                                        ${banReason === val ? "bg-red-500 text-white border-red-500" : "bg-zinc-50 text-zinc-600 border-zinc-200 hover:border-red-300"}`}>
                                        {label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* modal-note */}
                        <div className="mb-5">
                            <label className="block text-[12.5px] font-semibold text-zinc-500 mb-2">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setBanModal({ open: false, user: null })} className="flex-1 py-2.5 rounded-xl border border-zinc-200 text-[13px] font-semibold text-zinc-600 hover:bg-zinc-50 transition-colors">
                                Cancel
                            </button>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 border border-zinc-100">
                        ...
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                    <span className={`inline-flex text-[10.5px] font-semibold px-2 py-0.5 rounded-md ${ROLE_STYLE[detailModal.user.role]}`}>
                                    ...
                                    {detailModal.user.isActive ? (
                                        ...
                                        <span className="inline-flex items-center gap-1 text-[10.5px] font-semibold text-red-500 bg-red-50 px-2 py-0.5 rounded-md border border-red-200">
                        ...
                        {detailModal.user.driverProfile && (
                            <div className="bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 mb-4">
                                ...
                                <p className="text-[12.5px] text-teal-700">
                                    Status: <span className="font-bold">{detailModal.user.driverProfile.status}</span>
                                </p>
                            </div>
                        )}
                        {/* modal-ban-info */}
                            ...
                            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5">
                                ...
                                <div className="space-y-1">
                                    <p className="text-[12.5px] text-red-700">
                                        Reason: <span className="font-bold">{BAN_REASON_LABELS[detailModal.user.banReason]}</span>
                                    </p>
                                    {detailModal.user.bannedAt && (
                                        <p className="text-[12px] text-red-600">
                                            Banned on: {fmtDate(detailModal.user.bannedAt)}
                                        </p>
                                    )}
                                    {detailModal.user.banNote && (
                                        <p className="text-[12px] text-red-600 italic">"{detailModal.user.banNote}"</p>
                                    )}
                                </div>
                            </div>
                        )}
                        {/* modal-actions */}
                        <div className="flex gap-3">
                            <button onClick={() => setDetailModal({ open: false, user: null })}
                            ...
                            {detailModal.user.isActive ? (
                                <button onClick={() => { setDetailModal({ open: false, user: null }); setBanModal({ open: true, user: detailModal.user }); }}
                                    className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-[13px] font-semibold transition-all">
                                    Ban User
                                </button>
                            ) : (
                                <button onClick={() => { handleUnban(detailModal.user!.id); setDetailModal({ open: false, user: null }); }}
                                    className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-[13px] font-semibold transition-all">
                                    Unban User
                                </button>
                            )}
                        </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 border border-zinc-100">
                        ...
                        <div className="text-center mb-6">
                            ...
                            <p className="text-[12.5px] text-zinc-500">
                                <span className="font-semibold text-zinc-700">{deleteModal.user.name}</span> ko permanently delete karna chahte ho? Yeh action undo nahi ho sakta.
                            </p>
                        </div>
                        <div className="flex gap-3">
