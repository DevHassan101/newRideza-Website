...
interface HeaderProps { sidebarOpen: boolean; onToggle: () => void; }
...
export default function Header({ sidebarOpen, onToggle }: HeaderProps) {
    ...
    useEffect(() => {
        fetch("/api/auth/me")
            .then(r => r.json())
            .then(data => {
                if (data.user) setAdmin({ name: data.user.name, email: data.user.email });
            })
            .catch(() => {});
    }, []);
    ...
    return (
        <header className={`fixed top-0 right-0 h-20.25 bg-white border-b border-zinc-200 shadow-xs flex items-center px-6 gap-4 z-30 transition-all duration-300 
            ${sidebarOpen ? "lg:left-72 left-0" : "left-0"}`}>
            ...
            <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(!dropdownOpen)}
                    ...
                    <div className="text-left hidden sm:block">
                        {admin ? (
                            <>
                                <p className="text-sm font-bold text-zinc-800 leading-none">{admin.name}</p>
                                <p className="text-xs text-zinc-400 mt-0.5">{admin.email}</p>
                            </>
                        ) : (
                            <>
                                <div className="h-3.5 w-20 bg-zinc-200 rounded animate-pulse mb-1"/>
                                <div className="h-3 w-28 bg-zinc-200 rounded animate-pulse"/>
                            </>
                        )}
                    </div>
                    <svg className={`w-3.5 h-3.5 text-zinc-400 transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`}
                ...
                {dropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-zinc-100 rounded-2xl shadow-xl shadow-zinc-100 py-2 z-50">
                        ...
                        <div className="px-4 py-3 border-b border-zinc-100">
                            <p className="text-[13px] font-bold text-zinc-800 truncate">{admin?.name || "Admin"}</p>
                            <p className="text-[11px] text-zinc-400 mt-0.5 truncate">{admin?.email || "—"}</p>
                        </div>
                        {/* My Profile */}
                        <button onClick={() => { router.push("/admin/my-profile"); setDropdownOpen(false); }}
                        ...
                        {/* Settings */}
                        <button onClick={() => { router.push("/admin/settings"); setDropdownOpen(false); }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-zinc-600 hover:bg-zinc-50 hover:text-zinc-800 transition-colors">
                            <svg className="w-4 h-4 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
